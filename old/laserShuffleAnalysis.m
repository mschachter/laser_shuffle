function outVals = laserShuffleAnalysis(fullFileName, paramVals)
    ...
    [allData, delimVal, H] = importdata(fullFileName);
    numCols = size(allData.colheaders,2);
    cellCount = 0;
    laserCol = -1;
    numLaserCols = size(paramVals.laserColName, 2);
    laserTimes = [];
    % Cycle through all columns in the csv file
    for colNum = 1:numCols
        % Look for the column header that matches the laser times
        for laserColNum = 1:numLaserCols
            try
            if strcmp(allData.colheaders{colNum}, paramVals.laserColName{laserColNum})
                laserCol = colNum;
                % Take only the laser times that are real numbers (exclude filler)
                realVals = ~isnan(allData.data(:,laserCol));
                % Add offset to laser time exclude stimulus artifacts
                laserTimes = cat(1, laserTimes, allData.data(realVals,laserCol));
            end
            catch
                disp('problem');
            end
        end
        % Look through column headers to find columns of spike times    
        if (size(allData.colheaders{colNum}, 2) >= 3) && ...
                (strcmp(allData.colheaders{colNum}(1:3),'sig') && ...
                (strcmp(allData.colheaders{colNum}((end-2):end),'a_1') || ...
                strcmp(allData.colheaders{colNum}((end):end),'a')))
            %Add to the cell count, and add column number to list of cells
            cellCount = cellCount + 1;
            cellCol(cellCount) = colNum;
            cellName{cellCount} = allData.colheaders{colNum};

            % Transfer cell data to the list of allData
            realVals = ~isnan(allData.data(:,colNum));
            cellData{cellCount} = allData.data(realVals,colNum);
        end
    end
    ...
    if laserCol > 0
        ...
        numCells = -1;
        outVals.laserBinMaxSig = 0;
        outVals.laserBinMinSig = 0;
        outVals.laserMaxSigSum = 0;
        outVals.laserMinSigSum = 0;
        outVals.firstSigMaxBin = -1;
        outVals.psthBarVals = [];
        outVals.numCells = 0;
    end
    ...
    if paramVals.selectCells
        for cellNum = allCellNums
            disp([num2str(cellNum), '  --  ', cellName{cellNum}]);
        end
        badCellNum = 1;
        while badCellNum
            ...
            if badCellNum
                ...
                disp(['Reading cell: ', cellName{selCellNum}]);
            end 
        end
    ...
    %     disp(['Analyzing cell ', num2str(cellNum)]);
        ...
        title(rasterAx, ['Cell ', num2str(cellNum), ' of ', ...
            num2str(numCells), ': ', cellName{cellNum}], 'interpreter', 'none');
    ...
    %     xlimVals = get(plotAx,'xlim');
        ...
        outVals.cellName{cellNum} = cellName{cellNum};
